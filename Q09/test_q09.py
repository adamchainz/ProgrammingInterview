...
class Q09Tests(unittest.TestCase):
    def test_single(self):
        tree = TreeNode(5)
        self.assertEqual(nodes_at_depth(tree, 0), [5])
        self.assertEqual(nodes_at_depth(tree, 1), [])

    def test_simple(self):
        tree = TreeNode(4, TreeNode(1))
        self.assertEqual(nodes_at_depth(tree, 0), [4])
        self.assertEqual(nodes_at_depth(tree, 1), [1])
        self.assertEqual(nodes_at_depth(tree, 2), [])
